...
// else if (col==1) {vr_x=cos(2.0*PI*(120.0/360.0)); vr_y=sin(2.0*PI*(120.0/360.0));}    // detect green
    ...
    while (p != NULL) {
        ...
        dp = (vb_x * vr_x) + (vb_y *
                              vr_y);                                       // Dot product between the reference color vector, and the
        // blob's color vector.
        ...
        grayness = 0;
        if (fabs(p->R - p->G) / p->R < maxgray && fabs(p->R - p->G) / p->G < maxgray &&
            fabs(p->R - p->B) / p->R < maxgray && fabs(p->R - p->B) / p->B < maxgray && \
