...
#pragma omp parallel for schedule(dynamic, 16) private(i, j)
        for (j = 0; j < 768; j++)
            for (i = 0; i < 1024; i++) {
                *(big + (((i) + ((j + 128) * 1024)) * 3) + 0) = (unsigned char) ((*(blobIm->layers[0] + i +
                                                                                    (j * blobIm->sx))));
                *(big + (((i) + ((j + 128) * 1024)) * 3) + 1) = (unsigned char) ((*(blobIm->layers[1] + i +
                                                                                    (j * blobIm->sx))));
                *(big + (((i) + ((j + 128) * 1024)) * 3) + 2) = (unsigned char) ((*(blobIm->layers[2] + i +
                                                                                    (j * blobIm->sx))));
            }
        deleteImage(blobIm);
    }
...
struct image *renderBlobs(unsigned char *fgIm, int sx, int sy, struct image *labels, struct blob *list) {
    ...
    p = list;
    while (p != NULL) {
        if (p->label > maxLab) maxLab = p->label;
        p = p->next;
    }
...
void kbHandler(unsigned char key, int x, int y) {
    ...
    if (key == 'r') {
        setupAI(AIMode, botCol, &skynet);
        doAI = 0;
    }        // Resets the state of the AI (may need full reset)
    ...
    if (key == 'x') {
        ...
        if (got_Y == 0) {
            got_Y = 1;
            fprintf(stderr, "Capturing Y offset at center field\n");
        }
        else if (got_Y == 1) {
            got_Y = 2;
            fprintf(stderr, "Capturing Y offset at bottom for both robots\n");
        }
    }
    if (key == 'c') {
    ...
    if (key == '<') {
        bgThresh -= 50;
        fprintf(stderr, "BG subtract threshold now at %f\n", bgThresh);
    }
    if (key == '>') {
        bgThresh += 50;
        fprintf(stderr, "BG subtract threshold now at %f\n", bgThresh);
    }
    if (key == '{' && colAngThresh > .5) {
        colAngThresh -= .01;
        fprintf(stderr, "Colour Angle Threshold now at %f\n", colAngThresh);
    }
    if (key == '}' && colAngThresh < .99) {
        colAngThresh += .01;
        fprintf(stderr, "Colour Angle Threshold now at %f\n", colAngThresh);
    }
    if (key == '[' && colThresh >= 0.05) {
        colThresh -= .05;
        fprintf(stderr, "Saturation threshold now at %f\n", colThresh);
    }
    if (key == ']' && colThresh <= 0.95) {
        colThresh += .05;
        fprintf(stderr, "Saturation threshold now at %f\n", colThresh);
    }
    ...
    if (key == 'i') {
        if (DIR_FWD == 0) {
            ...
            DIR_R = 0;
            DIR_BACK = 0;
            BT_drive(LEFT_MOTOR, RIGHT_MOTOR, 75);
        }
        else {
            DIR_FWD = 0;
            BT_all_stop(0);
    ...
    if (key == 'j') {
        if (DIR_L == 0) {
            ...
            DIR_FWD = 0;
            DIR_BACK = 0;
            BT_turn(LEFT_MOTOR, 50, RIGHT_MOTOR, -50);
        }
        else {
            DIR_L = 0;
            BT_all_stop(0);
